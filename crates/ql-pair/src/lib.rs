...
use ql_core::{
    CertificateAuthority, DeviceCertificate, DeviceIdentity, KeyStorageLayout, QuantumLinkError,
    QuantumLinkResult, RevocationList,
};
...
const WORMHOLE_LEFT_WORDS: [&str; 64] = [
    "amber", "anchor", "apple", "aster", "atlas", "bamboo", "beacon", "birch", "bison", "blade",
    "blossom", "bluejay", "canyon", "cedar", "cipher", "clover", "comet", "coral", "cosmos",
    "cricket", "crystal", "dawn", "delta", "ember", "falcon", "fern", "fjord", "forest", "galaxy",
    "garden", "glacier", "granite", "guitar", "harbor", "hazel", "horizon", "island", "jasmine",
    "juniper", "lantern", "meadow", "meteor", "mist", "nebula", "oasis", "onyx", "orbit", "orchid",
    "otter", "phoenix", "pine", "prairie", "quartz", "raven", "river", "saffron", "sage",
    "sunrise", "tiger", "topaz", "violet", "willow", "zenith", "zephyr",
];

const WORMHOLE_RIGHT_WORDS: [&str; 64] = [
    "acorn", "arrow", "badger", "brook", "cabin", "canopy", "cardinal", "cascade", "castle",
    "circle", "cloud", "cobalt", "coyote", "drift", "echo", "elm", "feather", "field", "firefly",
    "flint", "gale", "geyser", "glow", "grove", "harvest", "heron", "hollow", "iris", "ivory",
    "lagoon", "leaf", "lilac", "lunar", "marble", "monsoon", "moon", "mountain", "nebula",
    "nectar", "nova", "ocean", "opal", "pebble", "petal", "reef", "ridge", "rook", "shadow",
    "shell", "signal", "solstice", "sparrow", "spruce", "stone", "stream", "summit", "thunder",
    "tidal", "trail", "valley", "voyage", "wave", "wind", "winter",
];
...
impl SignedDeviceCertificate {
    ...
    pub fn verify(
        ...
        verifying_key.verify(
            &certificate_signing_message(&self.certificate)?,
            &self.signature,
        )?;
...
impl QrPairingOffer {
    ...
    pub fn from_uri(uri: &str) -> QuantumLinkResult<Self> {
        let url = Url::parse(uri).map_err(|error| {
            QuantumLinkError::Pairing(format!("invalid QR pairing URI: {error}"))
        })?;
        ...
        let wg_public_key = decode_fixed_base64_32(params.get("wgkey").ok_or_else(|| {
            QuantumLinkError::Pairing("QR pairing URI missing wgkey".to_owned())
        })?)?;
        ...
        Ok(Self {
            ...
            rosenpass_fingerprint: params
                .get("rpfp")
                .ok_or_else(|| QuantumLinkError::Pairing("QR pairing URI missing rpfp".to_owned()))?
                .to_string(),
            rendezvous: params
                .get("rendezvous")
                .ok_or_else(|| {
...
impl WormholeCode {
    ...
    pub fn parse(code: &str) -> QuantumLinkResult<Self> {
        ...
        let number = number_part
            .parse::<u16>()
            .map_err(|_| invalid_wormhole_code())?;
        if number >= 1024
...
impl SetupKey {
    ...
    pub fn decode(encoded: &str) -> QuantumLinkResult<Self> {
        ...
        if parts.next() != Some(SETUP_KEY_PREFIX) {
            return Err(QuantumLinkError::Pairing(
                "invalid setup key prefix".to_owned(),
            ));
        }
        let expires_at = parts
            ...
            .parse::<u64>()
            .map_err(|error| {
                QuantumLinkError::Pairing(format!("invalid setup key expiry: {error}"))
            })?;
        let token = parts
            .next()
        ...
        if parts.next().is_some() || token.is_empty() {
            return Err(QuantumLinkError::Pairing(
                "invalid setup key format".to_owned(),
            ));
        }
...
fn decode_fixed_base64_32(input: &str) -> QuantumLinkResult<[u8; 32]> {
    ...
    bytes
        .try_into()
        .map_err(|_| QuantumLinkError::Pairing("expected a 32-byte base64 field".to_owned()))
}
...
mod tests {
    ...
    use super::{
        ...
        QrPairingOffer, SetupKey, SignedDeviceCertificate, WormholeCode, WormholePairingSession,
    };
