...
impl PeerState {
	fn refresh_relay_candidate(&mut self) {
		self.relay_candidate = self.preferred_relay_endpoint().map(|endpoint| PathCandidate {
			endpoint,
			latency_ms: self
				.relay_candidate
...
mod tests {
	...
	fn builds_mesh_peer_from_certificate() {
		...
		assert_eq!(peer.overlay_ip, "10.42.0.42".parse::<std::net::IpAddr>().unwrap());
	}
	...
	fn relay_policy_none_blocks_relay_fallback() {
		...
		assert_eq!(manager.path_for([9_u8; 32]).unwrap(), ConnectionPath::Unavailable);
	}
