...
use ql_gui::GuiModel;
use ql_macos_runtime::{MacOsAdapterExecution, MacOsRuntimeAdapter, MacOsRuntimeAdapterConfig};
...
impl MacOsAppShell {
    ...
    pub fn plan_connect_operations(
        ...
        let tunnel_request = tunnel.macos_bridge_request();
        let mut operations = vec![MacOsHostOperation::Tunnel(
            self.adapter
                .execute_tunnel_operation("activate", &tunnel_request)?,
        )];
        ...
        match firewall_policy {
            MacOsHostFirewallPolicy::KillSwitch { dns_server } => operations.push(
                MacOsHostOperation::Firewall(self.adapter.execute_firewall_operation(
                    "kill-switch",
                    &firewall.macos_kill_switch_request(dns_server, tunnel_request.peer_endpoint),
                )?),
            ),
    ...
    pub fn plan_disconnect_operations(
        ...
        Ok(vec![
            MacOsHostOperation::Firewall(
                self.adapter
                    .execute_firewall_operation("disable-all", &firewall.macos_disable_request())?,
            ),
...
mod tests {
    ...
    fn sample_tunnel() -> PlatformTunnel {
        PlatformTunnel::new(TunnelConfig {
            ...
            peer_endpoint: Some("198.51.100.8:51820".parse::<SocketAddr>().unwrap()),
    ...
    fn shell_plans_connect_operations() {
        ...
        let MacOsHostOperation::Firewall(execution) = &operations[1] else {
            panic!("expected firewall operation");
        };
        assert!(execution
            .payload
