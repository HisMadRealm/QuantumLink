...
impl PlatformFirewall {
	...
	#[cfg(target_os = "macos")]
	#[must_use]
	pub fn macos_kill_switch_request(&self, dns_server: IpAddr) -> MacOsFirewallBridgeRequest {
		MacOsFirewallBackend::new(&self.backend.interface)
			.bridge_request(MacOsFirewallOperationMode::KillSwitch, Some(dns_server))
	}
	...
	pub fn macos_dns_request(&self, dns_server: IpAddr) -> MacOsFirewallBridgeRequest {
		MacOsFirewallBackend::new(&self.backend.interface)
			.bridge_request(MacOsFirewallOperationMode::DnsOnly, Some(dns_server))
	}
	...
	pub fn macos_disable_request(&self) -> MacOsFirewallBridgeRequest {
		MacOsFirewallBackend::new(&self.backend.interface)
			.bridge_request(MacOsFirewallOperationMode::DisableAll, None)
	}
	...
	pub fn macos_query_request(&self) -> MacOsFirewallBridgeRequest {
		MacOsFirewallBackend::new(&self.backend.interface)
			.bridge_request(MacOsFirewallOperationMode::QueryActive, None)
	}
	...
	pub fn enable_kill_switch_with_executor<E: MacOsFirewallBridgeExecutor>(
		...
		executor.execute_firewall_request(&self.macos_kill_switch_request(dns_server))?;
		Ok(())
	}
...
impl MacOsFirewallBackend {
	fn new(interface: &str) -> Self {
		Self {
			...
			anchor_name: format!("com.quantumlink.{}", interface),
		}
	}

	fn enable(&self, mode: MacOsLeakProtectionMode, dns_server: IpAddr) -> QuantumLinkResult<()> {
		let _ = (&self.interface, &self.driver, &self.state, &self.anchor_name, mode, dns_server);
...
impl FirewallManager {
	...
	#[must_use]
	pub fn enable_kill_switch(&self, dns_server: IpAddr) -> QuantumLinkResult<()> {
		MacOsFirewallBackend::new(&self.interface).enable(MacOsLeakProtectionMode::KillSwitch, dns_server)
	}
	...
	#[must_use]
	pub fn enable_dns_protection(&self, dns_server: IpAddr) -> QuantumLinkResult<()> {
		MacOsFirewallBackend::new(&self.interface).enable(MacOsLeakProtectionMode::DnsOnly, dns_server)
	}
...
mod tests {
	...
	use super::{
		...
		MacOsFirewallDriver, MacOsFirewallOperationMode,
		MacOsFirewallState,
	};
	...
	fn macos_backend_initializes_native_shape() {
		...
		assert_eq!(backend.anchor_name, "com.quantumlink.ql0");
	...
	fn macos_bridge_request_carries_firewall_configuration() {
		...
		let request = firewall.macos_kill_switch_request(IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1)));

		assert_eq!(request.driver, "packet-filter");
		assert_eq!(request.anchor_name, "com.quantumlink.ql0");
		assert_eq!(request.interface_name, "ql0");
		assert_eq!(request.operation, MacOsFirewallOperationMode::KillSwitch);
		assert_eq!(request.dns_server, Some(IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1))));
	}
	...
	fn macos_executor_methods_consume_bridge_requests() {
		...
		firewall.enable_kill_switch_with_executor(&executor, dns).unwrap();
		firewall.enable_dns_protection_with_executor(&executor, dns).unwrap();
