...
mod tests {
    ...
    async fn forwards_packets_between_registered_peers() {
        ...
        let (bytes_read, source) =
            timeout(Duration::from_secs(1), responder.recv_from(&mut buffer))
                .await
                .unwrap()
                .unwrap();
        ...
        responder
            ...
            .await
            .unwrap();
        let (bytes_read, _) = timeout(Duration::from_secs(1), initiator.recv_from(&mut buffer))
            .await
            .unwrap()
            .unwrap();
    ...
    async fn ignores_unregistered_sources() {
        ...
        unknown
            .send_to(b"ignored", relay.local_addr())
            .await
            .unwrap();

        let mut buffer = [0_u8; 32];
        let recv_result =
            timeout(Duration::from_millis(200), receiver.recv_from(&mut buffer)).await;
