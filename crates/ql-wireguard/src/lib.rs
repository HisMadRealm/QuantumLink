...
pub struct TunnelConfig {
    ...
    /// Local interface addresses assigned to the tunnel.
...
/// Default bundle identifier for the packaged macOS Packet Tunnel Provider extension.
pub const DEFAULT_MACOS_TUNNEL_PROVIDER_BUNDLE_IDENTIFIER: &str =
    "com.quantumlink.macos.PacketTunnelProvider";
...
impl MacOsTunnelBackend {
    fn new(config: TunnelConfig) -> QuantumLinkResult<Self> {
        ...
        Ok(Self {
            provider_bundle_identifier: DEFAULT_MACOS_TUNNEL_PROVIDER_BUNDLE_IDENTIFIER.to_owned(),
            config,
    ...
    fn bring_up(&self) -> QuantumLinkResult<()> {
        let _ = (
            ...
            &self.driver,
            &self.state,
            &self.provider_bundle_identifier,
        );
    ...
    fn bridge_request(&self) -> MacOsTunnelBridgeRequest {
        MacOsTunnelBridgeRequest {
            ...
            interface_addresses: self.config.interface_addresses.clone(),
...
impl WireGuardTunnel {
    ...
    pub fn new(config: TunnelConfig) -> QuantumLinkResult<Self> {
        ...
        if existing_devices
            .iter()
            .any(|name| name == &config.interface_name)
        {
        ...
        route_socket
            .add_device(&config.interface_name)
            .map_err(|error| {
    ...
    pub fn bring_up(&self) -> QuantumLinkResult<()> {
        ...
        run_command(
            "ip",
            ["link", "set", "up", "dev", &self.config.interface_name],
        )?;
    ...
    pub fn tear_down(self) -> QuantumLinkResult<()> {
        ...
        route_socket
            .del_device(&self.config.interface_name)
            .map_err(|error| {
    ...
    pub fn stats(&self) -> QuantumLinkResult<TunnelStats> {
        ...
        let device = socket
            .get_device(wireguard_uapi::DeviceInterface::from_name(
                self.config.interface_name.as_str(),
            ))
            .map_err(|error| {
                QuantumLinkError::WireGuard(format!("failed to fetch interface state: {error}"))
            })?;
    ...
    fn configure_dns(&self) -> QuantumLinkResult<()> {
        ...
        *self
            .dns_state
            .lock()
            .map_err(|_| QuantumLinkError::WireGuard("dns state mutex poisoned".to_owned()))? =
...
fn validate_stub_config(config: &TunnelConfig) -> QuantumLinkResult<()> {
    ...
    if config.interface_addresses.is_empty() {
        return Err(QuantumLinkError::WireGuard(
            "at least one interface address is required".to_owned(),
        ));
    }
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    use super::{
        MacOsTunnelBackend, MacOsTunnelBridgeExecutor, MacOsTunnelBridgeRequest, MacOsTunnelDriver,
        MacOsTunnelState, TunnelStats, DEFAULT_MACOS_TUNNEL_PROVIDER_BUNDLE_IDENTIFIER,
    };
    use ipnetwork::IpNetwork;
    #[cfg(target_os = "macos")]
    ...
    fn macos_backend_initializes_native_shape() {
        ...
        assert_eq!(
            backend.provider_bundle_identifier,
            DEFAULT_MACOS_TUNNEL_PROVIDER_BUNDLE_IDENTIFIER
        );
    }
    ...
    fn macos_bridge_request_carries_tunnel_configuration() {
        ...
        assert_eq!(
            request.interface_addresses,
            vec![IpNetwork::V4("10.0.0.2/32".parse().unwrap())]
        );
        assert_eq!(request.private_key, [7_u8; 32]);
        assert_eq!(
            request.provider_bundle_identifier,
            DEFAULT_MACOS_TUNNEL_PROVIDER_BUNDLE_IDENTIFIER
        );
    ...
    impl MacOsTunnelBridgeExecutor for RecordingTunnelExecutor {
        fn activate_tunnel(&self, request: &MacOsTunnelBridgeRequest) -> QuantumLinkResult<()> {
            self.operations
                .borrow_mut()
                .push(format!("activate:{}", request.interface_name));
            Ok(())
        }
        ...
        fn deactivate_tunnel(&self, request: &MacOsTunnelBridgeRequest) -> QuantumLinkResult<()> {
            self.operations
                .borrow_mut()
                .push(format!("deactivate:{}", request.interface_name));
            Ok(())
        }
        ...
        fn update_tunnel_endpoint(
            ...
            self.operations
                .borrow_mut()
                .push(format!("endpoint:{}:{}", request.interface_name, endpoint));
            Ok(())
        }
        ...
        fn inject_tunnel_psk(
            ...
            self.operations
                .borrow_mut()
                .push(format!("psk:{}:{}", request.interface_name, psk[0]));
            Ok(())
        }
        ...
        fn read_tunnel_stats(
            ...
            self.operations
                .borrow_mut()
                .push(format!("stats:{}", request.interface_name));
            Ok(TunnelStats {
                bytes_sent: 11,
                bytes_received: 22,
                last_handshake: None,
    ...
    fn executor_assertions(tunnel: PlatformTunnel) {
        ...
        tunnel
            .update_peer_endpoint_with_executor(&executor, endpoint)
            .unwrap();
        tunnel
            .inject_psk_with_executor(&executor, [9_u8; 32])
            .unwrap();
