...
impl SignalClient {
    ...
    pub async fn send_mailbox_payload(
        &self,
        mailbox_id: Uuid,
        payload: Vec<u8>,
    ) -> QuantumLinkResult<()> {
    ...
    pub async fn receive_mailbox_payload(
        ...
        let response = self
            .send_request(
                self.http
                    .post(self.url(&format!("/mailbox/{mailbox_id}/recv"))),
            )
            .await?;
    ...
    pub async fn delete_mailbox(&self, mailbox_id: Uuid) -> QuantumLinkResult<()> {
        self.send_request(
            self.http
                .post(self.url(&format!("/mailbox/{mailbox_id}/delete"))),
        )
        .await?
        .error_for_status()
        .map_err(http_error)?;
    ...
    async fn send_request(
        ...
        let request = match &self.auth {
            ...
            SignalMailboxAuth::PairingId(pairing_id) => {
                request.header(PAIRING_ID_HEADER, pairing_id)
            }
        };
...
async fn list_peers(State(state): State<Arc<AppState>>, headers: HeaderMap) -> impl IntoResponse {
    let identity = match auth_identity(&headers) {
        Ok(identity) => identity,
        Err(response) => return response,
    };
...
async fn purge_expired_peers(state: &Arc<AppState>) {
    ...
    state
        ...
        .write()
        .await
        .retain(|_, peer| peer.updated_at.elapsed() <= peer_ttl);
}
...
async fn pop_message_for_recipient(
    ...
    let index = messages
        .iter()
        .position(|message| message.sender != recipient)?;
    messages.remove(index)
}

async fn cleanup_mailbox_if_complete(
    state: &Arc<AppState>,
    mailbox_id: Uuid,
    mailbox: &Arc<MailboxEntry>,
) {
