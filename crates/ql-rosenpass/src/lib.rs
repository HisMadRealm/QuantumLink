	...
	pub async fn stop(self) -> QuantumLinkResult<()> {
		...
		if let Some(mut child) = self.inner.child.lock().await.take() {
			...
			let _ = child.wait().await.map_err(|error| {
				QuantumLinkError::Rosenpass(format!("failed waiting for rosenpass child exit: {error}"))
			})?;
		}
		...
		let exit_status = {
			...
			match child_guard.as_mut() {
				Some(child) => match child.wait().await {
					...
					Err(error) => {
						set_last_error(&inner, format!("failed waiting on rosenpass child: {error}"));
						None
					}
				},
				None => None,
...
async fn spawn_rosenpass_process(inner: &Arc<Inner>) -> QuantumLinkResult<()> {
	...
	*inner.last_psk_rotation.lock().map_err(|_| {
		QuantumLinkError::Rosenpass("psk age mutex poisoned".to_owned())
	})? = Instant::now();
...
fn validate_config(config: &RosenpassConfig) -> QuantumLinkResult<()> {
	...
	for path in [&config.own_sk_path, &config.own_pk_path, &config.peer_pk_path] {
...
mod tests {
	...
	fn builds_exchange_command_arguments() {
		...
		assert_eq!(args.last().unwrap(), &BASE64_STANDARD.encode(config.peer_wg_pubkey));
	}
}
