...
use ql_core::{AlgorithmSet, ConnectionPath, DaemonCommand, DaemonEvent, PairingRole, TunnelState};
    ...
    pub fn apply_event(&mut self, event: DaemonEvent) {
        match event {
            ...
            DaemonEvent::MeshPeerDisconnected { peer_key } => {
                self.mesh_dashboard
                    .retain(|peer| peer.label != mesh_label(peer_key));
            }
...
mod tests {
    ...
    use ql_core::{
        AlgorithmSet, ConnectionPath, DaemonCommand, DaemonEvent, RelayPolicy, TunnelState,
    };
    use ql_mesh::MeshPeerStatus;
